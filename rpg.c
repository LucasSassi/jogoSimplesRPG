...
// tentar criar a pasta
        ...
        else
        {
            ...
            for (int y = 10; y < 54; y++)
            {
                for (int x = 10; x < 54; x++)
                {
                    if ((x - 32) * (x - 32) + (y - 24) * (y - 24) < 10 * 10)
                        ImageDrawPixel(&img, x, y, (Color){255, 200, 150, 255});
                }
            }
    ...
    if (playerIsAttacking)
    {
        ...
        if (playerAttackFrameCount > 1)
        {
            ...
            if (playerAttackFrameTime >= playerAttackFrameDuration)
            {
                ...
                playerAttackFrame++;
                if (playerAttackFrame >= playerAttackFrameCount)
                    playerAttackFrame = playerAttackFrameCount - 1;
            }
        }
    }
    if (bossIsAttacking)
    {
        ...
        if (bossAttackFrameCount > 1)
        {
            ...
            if (bossAttackFrameTime >= bossAttackFrameDuration)
            {
                ...
                bossAttackFrame++;
                if (bossAttackFrame >= bossAttackFrameCount)
                    bossAttackFrame = bossAttackFrameCount - 1;
            }
        }
    }
...
// SPRITES HUMANOIDES SIMPLES
void DrawPlayerSprite(int posX, int posY)
{
    // CABEÇA
    DrawCircle(posX + 10, posY - 12, 10, (Color){255, 220, 180, 255});
    DrawCircleLines(posX + 10, posY - 12, 10, BLACK);
    DrawCircle(posX + 7, posY - 14, 2, BLACK);
    DrawCircle(posX + 13, posY - 14, 2, BLACK);

    // PESCOÇO
    DrawRectangle(posX + 8, posY - 4, 4, 6, (Color){255, 220, 180, 255});

    // TORSO
    DrawRectangle(posX - 6, posY + 2, 32, 36, (Color){20, 130, 230, 255});
    DrawRectangleLines(posX - 6, posY + 2, 32, 36, (Color){0, 90, 170, 255});

    // OMBROS (circulos suaves)
    DrawCircle(posX - 2, posY + 6, 6, (Color){20, 130, 230, 255});
    DrawCircle(posX + 28, posY + 6, 6, (Color){20, 130, 230, 255});

    // BRAÇOS (articulados: ombro -> cotovelo -> mão)
    // braço esquerdo
    DrawLine(posX - 2, posY + 6, posX - 12, posY + 18, (Color){255, 220, 180, 255});
    DrawLine(posX - 12, posY + 18, posX - 6, posY + 26, (Color){255, 220, 180, 255});
    // braço direito
    DrawLine(posX + 28, posY + 6, posX + 38, posY + 18, (Color){255, 220, 180, 255});
    DrawLine(posX + 38, posY + 18, posX + 32, posY + 26, (Color){255, 220, 180, 255});

    // PERNAS (coxa -> joelho -> pé)
    // perna esquerda
    DrawLine(posX + 6, posY + 38, posX + 2, posY + 62, (Color){60, 60, 60, 255});
    DrawLine(posX + 2, posY + 62, posX + 6, posY + 72, (Color){60, 60, 60, 255});
    // perna direita
    DrawLine(posX + 18, posY + 38, posX + 22, posY + 62, (Color){60, 60, 60, 255});
    DrawLine(posX + 22, posY + 62, posX + 18, posY + 72, (Color){60, 60, 60, 255});
}

// CHEFE HUMANOIDE (MAIOR E MAIS IMPOSTO)
void DrawBossSprite(int posX, int posY)
{
    // AURA/SHADOW
    DrawCircle(posX + 20, posY + 36, 56, (Color){120, 20, 20, 90});

    // CABEÇA
    DrawCircle(posX + 20, posY - 8, 18, (Color){120, 40, 40, 255});
    DrawCircleLines(posX + 20, posY - 8, 18, (Color){200, 70, 70, 255});
    DrawCircle(posX + 12, posY - 12, 3, (Color){255, 180, 80, 255});
    DrawCircle(posX + 28, posY - 12, 3, (Color){255, 180, 80, 255});

    // PESCOÇO E OMBROS
    DrawRectangle(posX + 14, posY + 2, 12, 8, (Color){120, 40, 40, 255});
    DrawRectangle(posX + 2, posY + 6, 36, 44, (Color){170, 30, 30, 255});
    DrawRectangleLines(posX + 2, posY + 6, 36, 44, (Color){100, 10, 10, 255});

    // BRAÇOS GRANDES
    DrawLine(posX + 2, posY + 14, posX - 18, posY + 34, (Color){150, 40, 40, 255});
    DrawLine(posX - 18, posY + 34, posX - 12, posY + 54, (Color){150, 40, 40, 255});
    DrawLine(posX + 38, posY + 14, posX + 62, posY + 34, (Color){150, 40, 40, 255});
    DrawLine(posX + 62, posY + 34, posX + 56, posY + 54, (Color){150, 40, 40, 255});

    // PERNAS (mais grossas)
    DrawLine(posX + 10, posY + 50, posX + 6, posY + 90, (Color){90, 10, 10, 255});
    DrawLine(posX + 30, posY + 50, posX + 34, posY + 90, (Color){90, 10, 10, 255});
    DrawLine(posX + 6, posY + 90, posX + 2, posY + 100, (Color){40, 10, 10, 255});
    DrawLine(posX + 34, posY + 90, posX + 38, posY + 100, (Color){40, 10, 10, 255});
}
...
void DrawExplore(void)
{
    ...
    {
    case 0:
        storyText = "Voce chega na entrada da masmorra. O caminho esta fechado,\nvoce segue pela: Caverna umida ou Ponte antiga";
        break;
    case 1:
        storyText = "Voce entra em um salao com dois pedestais, voce toca o:\npedestal das chamas ou porta das sombras.";
        break;
    case 2:
        storyText = "Um guarda protege o caminho. Ir pelo tunel escuro\n ou usar o sino quebrado para distrai-lo.";
        break;
    case 3:
        storyText = "Voce esta cada vez mais perto: escolha um lado e avance. Lado bronze\n ou lado prata";
        break;
    }
...
void DrawBattle(void)
{
    ...
    // Desenhar chefe com primitivas humanoides (ignora texturas para garantir visual humanoide)
    DrawBossSprite((int)drawPosB.x - 20, (int)drawPosB.y - 60);
    bossDrawHeight = 120.0f; // altura aproximada do chefe desenhado
    ...
    // Desenhar jogador com primitivas humanoides (ignora texturas para garantir visual humanoide)
    ...
    for (int i = 0; i < INVENTORY_SIZE; i++)
    {
        ...
        if (inventory[i] == ITEM_SWORD || inventory[i] == ITEM_ARMOR)
            displayName = "Vazio";
...
int main(void)
{
    ...
    if (playerHitTexture.id)
        UnloadTexture(playerHitTexture);
    if (bossHitTexture.id)
        UnloadTexture(bossHitTexture);
